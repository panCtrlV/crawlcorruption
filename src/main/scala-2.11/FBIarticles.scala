 ...
 */
class FBIarticles extends Serializable {

  import com.impanchao.crawlcorruption.CrawledCorruptionArticle
...
zero"""
  ...
  def topWords(hdfsPath: String, top: Int) {
    sc.wholeTextFiles(hdfsPath).
      ...
      map(x => tokenize(x._2)).
      map(x => filterStopWords(x)).
      flatMap(x => x.map(y => (y, 1))).
      reduceByKey(_ + _).sortBy(y => (-y._2)).
      take(top)
  }

  def dictToTextFile(localPath: String, dict: Array[(String, Int)]): Unit = {
    val file: String = "corruptionDictionary.txt"
